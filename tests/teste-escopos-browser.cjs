...
(async()=>{
  ...
  try {
    ...
    await page.getByRole('button',{name:'+ Grupo na EAP',exact:true}).click();
    await page.getByLabel('Descrição da linha 1.0',{exact:true}).fill('Instalações elétricas');
    await page.getByRole('button',{name:'+ Item para cotação',exact:true}).click();
    await page.getByLabel('Descrição da linha 1.1',{exact:true}).fill('Organização do quadro elétrico e testes');
    await page.getByLabel('Quantidade da linha 1.1',{exact:true}).fill('1,5');
    await page.getByLabel('Unidade da linha 1.1',{exact:true}).selectOption('verba');
    await page.getByLabel('Marca ou referência da linha 1.1',{exact:true}).fill('Tigre');
    assert(await page.locator('#es-vinc-itens-1 input').isChecked(),'vínculo automático com único grupo');
    ...
    assert.equal(await page.getByLabel('Quantidade da linha 1.1',{exact:true}).inputValue(),'1.5');
    assert.equal(await page.getByLabel('Unidade da linha 1.1',{exact:true}).inputValue(),'verba');
    assert.equal(await page.getByLabel('Marca ou referência da linha 1.1',{exact:true}).inputValue(),'Tigre');
    await page.getByRole('button',{name:'Gerar planilha para fornecedor',exact:true}).click();
    await page.waitForFunction(()=>!esUI.ocupado);
    assert.match(await page.locator('#es-status').innerText(),/Planilha pronta/);
    assert.match(await page.getByRole('link',{name:'Baixar Excel (.xlsx)',exact:true}).getAttribute('href'),/export\?format=xlsx$/);
    assert.equal(app.planilhas[0].celulas['11,5'],'Tigre');assert.equal(app.planilhas[0].celulas['11,7'],'');
    await page.getByRole('button',{name:'Recuar nível da linha 1.1',exact:true}).click();
    assert(await page.getByLabel('Descrição da linha 2.0',{exact:true}).isVisible());
    await page.getByRole('button',{name:'Avançar nível da linha 2.0',exact:true}).click();
    assert(await page.getByLabel('Descrição da linha 1.1',{exact:true}).isVisible());
    assert(await page.locator('#es-planilha').isHidden(),'mudança oculta link de revisão anterior');
    ...
    await page.getByRole('button',{name:'Salvar rascunho',exact:true}).click();
    await page.waitForFunction(()=>!esUI.ocupado);
    await page.getByLabel('Descrição da linha 1.1',{exact:true}).evaluate(el=>{
      const data=new DataTransfer();data.setData('text/plain','Cabo elétrico\t2,5\tm\tPrysmian\nDisjuntor\t3\tun\tSchneider');
      el.dispatchEvent(new ClipboardEvent('paste',{clipboardData:data,bubbles:true,cancelable:true}));
    });
    assert.equal(await page.getByLabel('Descrição da linha 1.2',{exact:true}).inputValue(),'Disjuntor');
    assert.equal(await page.getByLabel('Marca ou referência da linha 1.2',{exact:true}).inputValue(),'Schneider');
    await page.getByRole('button',{name:'Gerar planilha para fornecedor',exact:true}).click();
    await page.waitForFunction(()=>!esUI.ocupado);
    assert.match(await page.locator('#es-status').innerText(),/Planilha pronta/);
    assert.equal(app.planilhas.at(-1).celulas['11,3'],2.5);assert.equal(app.planilhas.at(-1).celulas['12,3'],3);
