...
(async()=>{
  ...
  try {
    ...
    await page.locator('#es-grupos input').fill('Módulo 01 — Quadro elétrico — Térreo');
    ...
    await page.waitForFunction(()=>!esUI.ocupado);
    assert.equal(await page.locator('#es-fotos .es-foto').count(),1);
    ...
    await page.locator('#es-grupos .es-linha').nth(1).getByRole('button',{name:'Remover',exact:true}).click();
