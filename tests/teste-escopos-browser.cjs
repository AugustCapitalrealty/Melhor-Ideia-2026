...
(async()=>{
  ...
  try {
    ...
    await page.exposeFunction('esBackend',async(name,args)=>{
      if(name==='apiOpcoes')return {ok:true,empreendimentos:[{nome:'Mega Curitiba',empresa:'Demercado'}],categorias:[]};
      if(name==='apiObterReferenciaPrecos')return {ok:true,referencias:{}};
    ...
    // Importar a EAP na equalização real, sem preços herdados.
    await page.evaluate(()=>aba('nova'));
    await page.waitForFunction(()=>document.querySelector('#nEmp option[value="Mega Curitiba"]'));
    await page.getByRole('button',{name:'Carregar escopos',exact:true}).click();
    await page.waitForFunction(()=>escoposCotacao.length>0);
    const escolha=await page.evaluate(id=>String(escoposCotacao.findIndex(x=>x.id===id&&x.revisao===1)),id);
    await page.locator('#nEscopoOrigem').selectOption(escolha);
    await page.getByRole('button',{name:'Aplicar escopo na EAP',exact:true}).click();
    await page.waitForFunction(()=>!carregandoEscopoCotacao);
    assert.match(await page.locator('#msgEscopoCotacao').innerText(),/R1 carregado/);
    assert.equal(await page.locator('#nProjeto').inputValue(),'Adequações elétricas — teste de interface');
    assert.equal(await page.locator('#nEmp').inputValue(),'Mega Curitiba');
    assert(await page.locator('#chkModoMarcas').isChecked());
    const importacao=await page.evaluate(()=>({itens,proponentes}));
    assert.equal(importacao.itens[1].descricao,'Organização do quadro elétrico e testes','revisão antiga, não os itens colados depois');
    assert.equal(importacao.itens[1].marcaReferencia,'Tigre');assert.equal(importacao.itens[1].quantidade,'1.5');
    assert.equal(importacao.itens[1].precos.length,importacao.proponentes.length);
    assert(importacao.itens.every(it=>it.precos.every(v=>v==='')));
    assert.match(await page.locator('#nDetalhamento').inputValue(),/Revisão 1/);
    await page.evaluate(()=>{itens[1].precos[0]='100';desenharGrade();});
    page.once('dialog',dialog=>dialog.dismiss());
    await page.getByRole('button',{name:'Aplicar escopo na EAP',exact:true}).click();
    assert.equal(await page.evaluate(()=>itens[1].precos[0]),'100','cancelar mantém preços existentes');
    // Erro do servidor deve manter a grade e liberar os controles.
    await page.evaluate(()=>{escoposCotacao[Number(document.getElementById('nEscopoOrigem').value)].revisao=999;});
    page.once('dialog',dialog=>dialog.accept());
    await page.getByRole('button',{name:'Aplicar escopo na EAP',exact:true}).click();
    await page.waitForFunction(()=>!carregandoEscopoCotacao);
    assert.match(await page.locator('#msgEscopoCotacao').innerText(),/Seus dados foram mantidos/);
    assert.equal(await page.evaluate(()=>itens[1].precos[0]),'100');
    assert(!(await page.locator('#btAplicarEscopo').isDisabled()));
    await page.evaluate(()=>{escoposCotacao[Number(document.getElementById('nEscopoOrigem').value)].revisao=1;proponentes[0].nome='Fornecedor existente';proponentes[0].totalDeclarado='999';proponentes[0].propostaInicial='1200';});
    page.once('dialog',dialog=>dialog.accept());
    await page.getByRole('button',{name:'Aplicar escopo na EAP',exact:true}).click();
    await page.waitForFunction(()=>!carregandoEscopoCotacao);
    assert.equal(await page.evaluate(()=>itens[1].precos[0]),'');
    assert.equal(await page.evaluate(()=>proponentes[0].totalDeclarado),'');
    assert.equal(await page.evaluate(()=>proponentes[0].propostaInicial),'');
    assert.equal(await page.evaluate(()=>proponentes[0].nome),'Fornecedor existente');
    assert.deepEqual(errors,[],'sem erros na importação para equalização');
